...
# read in the data 
ww_quasi <- fread("snpcall_benchmark_DELTA.txt")
ww_quasi_1 <- fread("snpcall_benchmark_DELTA_1.txt")
head(ww_quasi)
head(ww_quasi_1)
ww_quasi_filtered <-  ww_quasi %>% filter(!str_detect(caller,"GATK"))

write.csv(ww_quasi_filtered, file="snpcall_benchmark_DELTA_1_filtered.txt")
...
avg_freq <- fread("WWC092_Delta_samp_code_mean_frequency.txt")
...
ww_quasi_caller <- left_join(ww_quasi_caller, avg_freq, by='samp_code')

#plotting all the samples, colored by caller
point_plot <- ggplot(ww_quasi_caller, aes(precision, recall, color=AVG_Freq)) +
  geom_point(size=2.5) +
...
point_plot 
ggsave("WW092_DELTA_color_by_caller_600_1.tiff",units="in", width= 12, height = 6, device='tiff', dpi=600)
ggsave("WW092_DELTA_color_by_caller_300_1.tiff",units="in", width= 12, height = 6, device='tiff', dpi=300)
ggsave("WW092_DELTA_color_by_caller_600_1.pdf",units="in", width= 12, height = 6, device='pdf', dpi=600)
ggsave("WW092_DELTA_color_by_caller_300_1.png",units="in", width= 12, height = 6, device='png', dpi=300)

#plotting all the samples, grouped by caller with trend line
point_plot <- ggplot(ww_quasi_caller, aes(precision, recall)) +
  geom_point(size=2.5) +
...
point_plot
ggsave("WW092_DELTA_color_by_caller_multi_600.tiff",units="in", width= 12, height = 6, device='tiff', dpi=600)
ggsave("WW092_DELTA_color_by_caller_multi_300.tiff",units="in", width= 12, height = 6, device='tiff', dpi=300)
ggsave("WW092_DELTA_color_by_caller_multi_600.pdf",units="in", width= 12, height = 6, device='pdf', dpi=600)
ggsave("WW092_DELTA_color_by_caller_multi_300.png",units="in", width= 12, height = 6, device='png', dpi=300)

#plotting all the samples, grouped by caller with trend with label() sample_code)
point_plot <- ggplot(ww_quasi_caller, aes(precision, recall, label=samp_code)) +
  geom_point(size=2.5) +
  theme(legend.spacing.x = unit(0.2, 'cm'), text = element_text(size=8)) +
  theme_bw(base_size=15) + coord_cartesian(xlim=c(0,1.0), ylim=c(0,1.0)) +
  theme(legend.position = "bottom") + facet_wrap(~caller) 
point_plot + geom_text_repel()
ggsave("WW092_DELTA_color_by_caller_multi_label_600.tiff",units="in", width= 12, height = 6, device='tiff', dpi=600)
ggsave("WW092_DELTA_color_by_caller_multi_label_300.tiff",units="in", width= 12, height = 6, device='tiff', dpi=300)
ggsave("WW092_DELTA_color_by_caller_multi_label_600.pdf",units="in", width= 12, height = 6, device='pdf', dpi=600)
ggsave("WW092_DELTA_color_by_caller_multi_label_300.png",units="in", width= 12, height = 6, device='png', dpi=300)
...
point_plot
ggsave("WW092_DELTA_color_by_caller_recall_300_box_jitterpoints.png",units="in", width= 12, height = 6, device='png', dpi=300)
...
point_plot <- ggplot(ww_quasi_caller, aes(precision, caller, color=caller)) +
  ...
  theme(legend.position = "bottom")  
point_plot
ggsave("WW092_DELTA_color_by_caller_pre_300_box_jitterpoints.png",units="in", width= 12, height = 6, device='png', dpi=300)

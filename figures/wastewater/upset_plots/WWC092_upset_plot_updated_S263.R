...
write.table(POS_comb, "POS_comb_S263.txt",na="", sep="\t")
#write.table(POS_comb, "POS_comb_S263_added.txt",na="", sep="\t")
#Delta columne and values added to written file

POS_comb_added <- fread("POS_comb_S263_added.txt")
...
write.xlsx(text_list, "S263_binary.xlsx")
...
count <- data.frame(Caller = c("LoFreq","iVar","GATK","FreeBayes","BCFtools","VarScan"), 
                    ...
                    Found_in_S263_OmicronBA.1 = c(LoFreq_OmicronBA.1,iVar_OmicronBA.1,GATK_OmicronBA.1,FreeBayes_OmicronBA.1,BCFtools_OmicronBA.1,VarScan_OmicronBA.1), 
                    Found_in_S263_OmicronBA.2 = c(LoFreq_OmicronBA.2,iVar_OmicronBA.2,GATK_OmicronBA.2,FreeBayes_OmicronBA.2,BCFtools_OmicronBA.2,VarScan_OmicronBA.2), 
                    Found_in_S263_Delta = c(LoFreq_Delta, iVar_Delta, GATK_Delta, FreeBayes_Delta, BCFtools_Delta, VarScan_Delta))
print(count)
write.xlsx(count, "Found_S263_count.xlsx")

#Plot upset plot with parameters
upset(fromList(POS_comb_added), nintersects=99, cutoff=9, keep.order=T, sets = c("LoFreq","iVar","GATK","FreeBayes","BCFtools","VarScan","Delta","Omicron BA.2","Omicron BA.1"), nsets=9, point.size=2.5, line.size=1, order.by = "degree", number.angles= 0, mainbar.y.label= "No of SNPs/Indels intersections", sets.x.label = "S263 - No of SNPs/Indels", text.scale = 1.75, sets.bar.color = "blue", main.bar.color = "dark green", matrix.color = "red")


ggsave("Upset_plot_WWC092_S263.png",units="in", width= 12, height = 6, device='png', dpi=300)
...
ggsave("Upset_plot_WWC092_S263_wo_lfq.png",units="in", width= 12, height = 6, device='png', dpi=300)
